...
def test_print():
    ...
    e = Element('O')
    assert str(e) == "Element(O A=15.999 Z=8.0 N=8.004 e=8.0)"
    assert str((e.Z, e.N, e.e, e.A)) == "(8.0, 8.00448, 8.0, Quantity(1.600e+01 Da))"

    e = Element('O', natural=False)
    assert str(e) == "Element(O A=15.995 Z=8 N=8.000 e=8)"
    
    e = Element('O', count=2, natural=False)
    assert str(e) == "Element(O2 A=31.990 Z=16 N=16.000 e=16)"
    assert e.count == 2
    assert e.element == 'O'
    assert e.isotope == 16
    ...
    e = Element('O{17-2}')
    assert str(e) == "Element(O{17-2} A=16.998 Z=8 N=9.000 e=6)"
