...
def test_empty():
    material = Material()
    assert material.data_substances() == None
    
def test_set_dictionary():
    material = Material({
        'H2O': 1.0
    })
    assert material.data_substances(quantity=False).to_text() == """
...
def test_norm_number():
    ...
    assert material.data_material(quantity=False).to_text() == """
   expr      x           X
...
def test_norm_mass_fraction():
    ...
    material = Material('0.2 <H2O> 0.3 <NaCl>', norm=Norm.MASS_FRACTION)
    assert material.data_material(quantity=False).to_text() == """
   expr           x      X
...
""".strip('\n')
    ...
    material = Material('0.683815 <H2O> 0.316185 <NaCl>', norm=Norm.NUMBER_FRACTION)
    assert material.data_material(quantity=False).to_text() == """
   expr         x           X
...
def test_natural_abundances():
    ...
    assert isclose(
        material.data_substances(quantity=False).H2O.mass, 
        18.015286, 
...
def test_example():
    m = Material({
       ...
       'Ar':  0.93400,
       'CO2': 0.03600,
    }, norm=Norm.NUMBER_FRACTION)
    assert m.data_substances(quantity=False).to_text() == """
  expr  fraction       mass     Z          N     e
...
""".strip('\n')
    assert m.data_material(quantity=False).to_text() == """
  expr        x           X
